...
const MetaMaskForm = ({ metaMaskFormMethods, onMetaMaskFormSubmit, hidden, loading, assetDetails, getNetworkPrice, bidAmount }) => {
  ...
  return (
    <Form hidden={hidden} autoComplete="off">
      {loading && <div className="invalidFeedback">Please do not refresh the page</div>}
      {payBtnDisable && 'Insufficient balance in your wallet'}

      <Form.Group className="form-group">
        <Form.Label>Asset Price</Form.Label>
        <Form.Control
          type="text"
          value={`£ ${assetGbpPrice} (${getNetworkPrice(assetGbpPrice)} ${getNetworkSymbol(
      ...
      >
        {loading ? 'Payment in progress...' : payBtnDisable ? 'Please fund your wallet' : 'Pay now'}
