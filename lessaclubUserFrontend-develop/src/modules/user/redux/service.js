...
export const getUserBankAccountDetails = (id) => (dispatch) => {
  dispatch({ type: CLEAR_USER_BANK_ACCOUNT_DETAILS_RESPONSE })
  axios
    .get(`${apiPaths.userBankAccount}/${id}`)
    .then(({ data }) => {
      dispatch({
        type: GET_USER_BANK_ACCOUNT_DETAILS,
        payload: {
          data: data.result,
          resMessage: data.message,
          resStatus: true
