...
const AuctionBidList = ({ getNetworkPrice, assetDetails }) => {
  ...
  const networkSymbol = getNetworkSymbol(assetDetails?.blockchainNetwork || NETWORKS.ETHEREUM)

  return (
    <>
      <div className="asset-detail-list" id="asset-bid-list">
          ...
          return (
            <div key={index} className="asset-single-desc d-flex justify-content-between flex-wrap">
              <div className="single-desc-left d-flex align-items-center">
                <img src={bid?.user?.profilePicUrl || userProfileIcon} className="img-fluid flex-shrink-0" alt="owner-img" />
              ...
              <div className="single-desc-right">
                <h5>
                  {getNetworkPrice(bid?.amount, networkSymbol)} {networkSymbol}
                </h5>
        ...
        {(!bids || !bids?.bid || (bids?.bid && bids?.bid.length === 0)) && (
          <p>
            <Button className="disable-red-btn" disabled>
              <FormattedMessage id="nobodyBidded" />
            </Button>
          </p>
        )}
      </div>
