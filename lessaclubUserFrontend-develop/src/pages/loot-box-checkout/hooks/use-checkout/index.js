...
import { getLootBoxDetails } from 'modules/crates/redux/service'
...
const useCheckout = () => {
  ...
  const lootBoxDetailStore = useSelector((state) => state.crates.lootBoxDetails)
  ...
  useEffect(() => {
    if (userData && assetDetails) {
      ...
      dispatch(getPendingOrder({ lootBoxId: assetDetails?.id, userId: userData?.id, pendingOrder: true }))
    }
  ...
  useEffect(() => {
    if (lootBoxDetailStore) {
      setAssetDetails(lootBoxDetailStore?.lootBox)
    }
  }, [lootBoxDetailStore])

  useEffect(() => {
    id && dispatch(getLootBoxDetails(id))
  }, [id])
  ...
  useEffect(() => {
    if (userStore && userStore.id) {
      dispatch(getReferralDiscount(id, { type: 'lootBox' }))
    }
  }, [userStore])
