...
const useOrders = () => {
  const { id } = useParams()
  const path = window.location.pathname.split('/profile/')[1]
  ...
  function getRequestParams(e) {
    ...
    const paramValue = path === 'order-to-send' ? 'sellerId' : 'userId'
    return {
      [paramValue]: id || localStorage.getItem('userId'),
      page: data.page || 1,
  ...
  const handleOrderChange = (data, isFilter) => {
    if (isFilter) {
      setRequestParams({ ...requestParams, statusId: data ? data?.value : '' })
      appendParams({ statusId: data ? data?.value : '' })
    } else {
  ...
  const handleOrderTypeFilter = (data) => {
    setRequestParams({ ...requestParams, orderType: data || '' })
    appendParams({ orderType: data || '' })
  }
