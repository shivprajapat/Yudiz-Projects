  ...
  return (
    <Layout
      data={{
        oSeo: {
          ...seoData?.getSeoData,
          sTitle: "ICC Men's Cricket Rankings | Men's Player & Team Rankings",
          sDescription: 'Check out the latest ICC mens ODI cricket rankings, mens test cricket rankings, mens t20 cricket rankings of teams & players on CricTracker'
        }
      }}
    >
                    ...
                    return (
                      <tr key={el._id} className={`${el.highlight && 'highlight'}`}>
                        ...
                        {filter.eRankType !== 'Teams' && (
                          <td>
                            {width < 767 ? (el?.oPlayer?.sCountry || el?.oPlayer?.sCountryFull || el?.sTeam) : (el?.oPlayer?.sCountryFull || el?.oPlayer?.sCountry || el?.sTeam) || '-'}
                          </td>
                        )}
                        <td>{el.nRating}</td>
                        {filter.eRankType === 'Teams' && <td>{el.nPoints}</td>}
                      </tr>
                    )
                  })}
                </ThemeTable>
