...
function ArticleMedium({ data, isVideo }) {
  ...
  return (
    <ArticleStyles>
      {(articleStyles) => (
        <article id={data?._id} className={`${articleStyles.article} ${styles.articleMedium} ${isVideo && articleStyles.video} light-bg br-lg c-transition`}>
          <Row className="gx-2 gx-md-3">
            <Col xs={5} sm={6}>
              <CustomLink href={`/${data?.oSeo?.sSlug || '404'}`} prefetch={false}>
                <a className={`${articleStyles.postimg} ${styles.postimg} d-block br-md overflow-hidden position-relative a-transition`}>
                  {isVideo && (
                    <MyImage
            ...
            <Col xs={7} sm={6} className="d-flex flex-column justify-content-between">
              <div>
                ...
                {data?.sDescription && <p className={`${articleStyles.desc} ${styles.desc} small-text text-secondary line-clamp-3 overflow-hidden`}>{data?.sDescription}</p>}
              </div>
