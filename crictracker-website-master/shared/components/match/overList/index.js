...
const OverList = ({ data, id }) => {
  ...
  return (
    <section className={`${styles.overList}`} id={id}>
      <div className={`${styles.item} common-box d-flex align-items-center px-2 py-3 p-sm-3 p-xl-4 mb-3`}>
        <div className={`${styles.over} text-center`}>
          <p className={`${styles.overNo} mb-2 font-semi`}>{data?.sOver} {t('common:Over')}</p>
          <div className={`${styles.overRun} fw-bold br-sm`}>{data?.nOverTotal} {t('common:Runs')}</div>
        </div>
            ...
            {data?.aBall?.map((ball, index) => {
              return (
                <div className={`${styles.ball}`} key={index}>
                  ...
                  {ball?.sScore === '6' && <div className={`${styles.run} ${styles.runPrimary} rounded-pill bg-primary text-white`}>{ball?.sScore}</div>}
