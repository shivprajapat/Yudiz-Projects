...
import { monthCal, currentMonthYear, decodeMonth, getTimeZone, isBottomReached } from '@utils'
...
export default function ScheduleContent({ data, filterData }) {
  ...
  const payloads = useRef({ eCategory: 'i', dByMonth: currentMonthYear(), sTimezone: getTimeZone(), nLimit: 10, nSkip: 1 })
  ...
  const latestScheduleMatch = useRef(updateScheduleMatchData?.listAllFixtures?.length || data?.listAllFixtures?.length)
  ...
  const handleCategoryFilter = (value) => {
    payloads.current = { ...payloads.current, eCategory: value, nSkip: 1 }
    filterPayloads.current = { ...filterPayloads.current, eCategory: value }
    update(true)
  }

  const handleYear = (value) => {
    payloads.current = { ...payloads.current, dByMonth: decodeMonth(value), nSkip: 1 }
    filterPayloads.current = { ...filterPayloads.current, dByMonth: decodeMonth(value) }
    update(true)
  }

  const handleClear = () => {
    payloads.current = { ...payloads.current, iVenueId: null, iSeriesId: null, iTeamId: null, nSkip: 1 }
  ...
  const handleFilterChange = (data, type) => {
    if (type === 'venue') {
      payloads.current = { ...payloads.current, iVenueId: data === null ? null : data.sValue, nSkip: 1 }
      filterPayloads.current = { ...filterPayloads.current, iVenueId: data === null ? null : data.sValue }
    } else if (type === 'team') {
      payloads.current = { ...payloads.current, iTeamId: data === null ? null : data.sValue, nSkip: 1 }
      filterPayloads.current = { ...filterPayloads.current, iTeamId: data === null ? null : data.sValue }
    } else if (type === 'series') {
      payloads.current = { ...payloads.current, iSeriesId: data === null ? null : data.sValue, nSkip: 1 }
  ...
  useEffect(() => {
    ...
    isBottomReached(fixtureData[fixtureData.length - 1]?._id, isReached)
  }, [fixtureData])

  function isReached(reach) {
    if (reach && !loadingData.current && latestScheduleMatch.current === payloads.current.nLimit) {
  ...
  useEffect(() => {
    if (isReachedBottom.current && updateScheduleMatchData) {
      setFixtureData([...fixtureData, ...updateScheduleMatchData?.listAllFixtures])
      loadingData.current = false
      isReachedBottom.current = false
    } else if (!loading.current && updateScheduleMatchData) {
      setFixtureData(updateScheduleMatchData?.listAllFixtures)
    }
    latestScheduleMatch.current = updateScheduleMatchData?.listAllFixtures?.length || data?.listAllFixtures?.length
  ...
  return (
    <div className="position-relative">
      ...
      {width < 768 && (<>
        <div className={`${mobilefilter.mobFixtureFilter} ${toggleFilter ? mobilefilter.active : ''} light-bg position-fixed vw-100`}>
          ...
          <div className={`${mobilefilter.innerBlock} d-block d-md-none`}>
            <NameFilter formStyles={formStyles} isMobile mobilefilter={mobilefilter} fixtureData={fixtureData} handleClear={handleClear} filterData={newFilterData} handleFilterChange={handleFilterChange} />
          </div>
        </div>
        <Button
          variant="link"
      ...
      <DateSlider monthArray={monthArray} selectedMonth={payloads.current.dByMonth} handleYear={handleYear} isSticky />
