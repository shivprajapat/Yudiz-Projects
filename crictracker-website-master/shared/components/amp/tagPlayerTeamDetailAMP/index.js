...
function TagPlayerTeamDetailAMP({ tag, seoData }) {
  ...
  } else if (router?.query?.tab === 'fantasyArticle') {
    return <SeriesHomeAMP data={{ fantasyArticle: tag?.home?.fantasyArticle }} category={tag?.data} isClientOnly showMoreBtn />
  } else {
    return <SeriesHomeAMP data={tag?.home} category={tag?.data} isClientOnly showMoreBtn />
  }
}
