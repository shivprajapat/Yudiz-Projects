...
const CurrentFutureSeriesList = ({ data }) => {
  return (
    ...
    {data?.length !== 0 && <div className={styles.archiveList}>
      <div className={`${styles.item} common-box d-flex align-items-center bg-secondary text-light fw-bold mb-2`}>
      ...
      {data?.map((seriesData, index) => {
        return (
          <div key={index} className={`${styles.item} common-box d-flex mb-2 br-md`}>
                ...
                return (
                  <React.Fragment key={series._id}>
                    <div className={`${styles.seriesItem} d-md-flex justify-content-md-between`}>
                      <p className="font-semi mb-1 mb-md-0 flex-grow-1"><CustomLink href={series?.oCategory?.oSeo?.sSlug || series?.oSeo?.sSlug || ''} prefetch={false}><a>{series?.sTitle}</a></CustomLink></p>
