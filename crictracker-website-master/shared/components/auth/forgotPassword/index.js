...
function ForgotPassword() {
  ...
  const onSubmit = (data) => {
    ...
    if (openOtp && sCode) {
      ...
      resendData.current = { eAuth: 'fr', eType: CheckLoginType, sLogin: data.sLogin }
    }
  }
                  ...
                  <Col md={7} className={`${styles.innerSmall}`}>
                    ...
                    <Form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
                      ...
                      <Button
                        ...
                        variant="primary"
                        type="submit"
                        disabled={sendOtpLoading || verifyOtpLoading || (openOtp && !sCode)}
                      >
                        {t('common:Submit')}
                        {(sendOtpLoading || verifyOtpLoading) && <Spinner className="ms-2 align-middle" animation="border" />}
                      </Button>
