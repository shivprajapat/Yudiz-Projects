...
function TopPlaylist({ data }) {
  ...
  return (
    <section className={`${styles.topPlaylist} py-3 pt-0`}>
      <h4 className="text-uppercase">{t('common:TopPlaylist')}</h4>
      <div className={`${styles.items} d-flex text-center pb-3 overflow-hidden br-lg`}>
        {data?.map((element) => {
          return (
            <CustomLink href={`${element?.oSeo?.sSlug}/${element?.oCategory?.eType !== 's' && element?.oSeo?.eType !== 'pl' ? `${allRoutes.cricketVideo}` : '?tab=videos'}`} key={element?._id} prefetch={false}>
              <a className={`${styles.item} d-flex align-items-center flex-shrink-0 flex-lg-shrink-1`}>
                <div className="w-100">
                  ...
                  {
                    !element?.sThumbnailUrl &&
                    <p className="big-text fw-bold text-light text-uppercase mb-0 p-2 p-xl-3">{element?.sTitle}</p>
                  }
                </div>
              </a>
            </CustomLink>
