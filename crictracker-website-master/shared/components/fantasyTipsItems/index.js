...
const FantasyTipsItems = ({ data, isSeries }) => {
  ...
  return (
    <div className={`${styles.fantasyTips} mt-4`} id={data?._id}>
      {!isSeries ? (
        <CustomLink href={`/${data?.oSeries?.oSeo?.sSlug}`}>
          <a className='text-primary font-semi mb-2 d-inline-block text-decoration-underline'>{data?.oSeries?.sTitle}</a>
        </CustomLink>
      ) : (
        <p className="font-semi mb-2">{convertDt24h(data?.dStartDate)}</p>
      )}
          ...
          <div className={`${styles.matchInfo} col-md-5 pe-md-2`}>
            ...
            <div className={`${styles.teams} d-flex align-items-center justify-content-center`}>
              <div className={`${styles.team} d-flex align-items-center`}>
                ...
                <p className="big-text font-semi mb-0">{data?.oTeamA?.sAbbr || data?.oTeamA?.sTitle}</p>
              </div>
              ...
              <div className={`${styles.team} d-flex align-items-center`}>
                ...
                <p className="big-text font-semi mb-0">{data?.oTeamB?.sAbbr || data?.oTeamB?.sTitle}</p>
              </div>
            </div>
                      ...
                      {platform?.ePlatformType === 'de' && (
                        <CustomLink href={`/${platform?.oSeo?.sSlug || ''}`} prefetch={false}>
                          <a className={`${styles.tipsBtn} ${styles.dream11} d-flex align-items-center mb-1 mt-1 mb-md-2 mt-sm-2`}>
                            <span className={`${styles.icon} d-block me-2 rounded-circle overflow-hidden`}>
