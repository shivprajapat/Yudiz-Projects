...
const Comments = ({ handleComments, showComments, commentCount, setCommentCount, articleId, count, type, isRequireAdminApproval }) => {
    ...
    if (!isConfirm) {
      ...
      if (type === 'article') {
        ...
        if (data?.deleteFrontComment) {
          ...
          setCommentCount(commentCount !== 0 && commentCount - 1)
        }
    ...
    <>
      <Offcanvas className={styles.offcanvasComments} show={showComments} onHide={handleComments} placement={'end'}>
        <div className={`${styles.offcanvasHead} d-flex justify-content-between`}>
          <p className="big-text fw-bold mb-0">
        ...
        <Offcanvas.Body className={styles.offcanvasBody} onScroll={handleScroll}>
          <div className={styles.commentsGroup}>
            <Form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
              <CustomFormGroup className={`${formStyles.formGroup} ${styles.formGroup} `} controlId="phoneNumber">
                <CustomInput
                  className={`${formStyles.formControl} ${formStyles.formTextarea} ${styles.commentsControl} ${errors.sMessage && formStyles.hasError
                    }`}
                  as="textarea"
                  disabled={!token || token === 'undefined' || token === 'null'}
                  name="sMessage"
              ...
              {(!token || token === 'undefined' || token === 'null') ? (
                <div className={`${styles.invalidFeedback} d-flex align-items-center`}>
                  <CustomFeedback message={validationErrors.pleaseLoginToWriteComment} type="invalid" className={`${formStyles.invalidFeedback} m-0`} />
                  <CustomLink href={allRoutes.signIn}>
                    <a className="theme-btn small-btn ms-2">{t('common:SignIn')}</a>
                  </CustomLink>
                </div>
              ) : <>
                <Button
                ...
                <Button
                  disabled={addCommentLoading || !token || token === 'undefined' || token === 'null' || addFantasyArticleCommentLoading}
      ...
      <Modal show={isReport} onHide={handleReport} centered className="modal-medium">
        <Modal.Body className={`${styles.modalBody} mx-auto mt-2`}>
          ...
          <Form onSubmit={handleSubmitReport(onReportSubmit)} autoComplete="off">
            <CustomFormGroup className={`${formStyles.formGroup}`} >
              {reportProblemData?.map((report) => {
                return (
                  <Form.Check
                    ...
                    label={report.sTitle}
                    value={report._id}
                    className={`${styles.formCheck} ${formStyles.formRadio} ${formStyles.formRadioCheck} ${formStyles.bigcheck} font-semi`}
              ...
              {reportErrors.iReportReasonId && (
                <CustomFeedback message={reportErrors.iReportReasonId.message} type="invalid" className={`${formStyles.invalidFeedback}`} />
              )}
            </CustomFormGroup>
