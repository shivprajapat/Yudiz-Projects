...
function GlobalPoll({ userPermission }) {
  ...
  const bulkActionDropDown = [
    { label: 'Delete All', value: 'd', isAllowedTo: 'DELETE_POLL' }
  ]
  ...
  function getActionColumns() {
    ...
    const clm = [
      ...
      { name: <FormattedMessage id="maxAndTotalVotes" />, internalName: 'sSlug', type: 0 }
    ]
    return clm.map((e) => {
  ...
  function handleTabChange(name) {
    if (name === 'scheduled') {
      setRequestParams({ ...requestParams, aStatus: ['s'], nSkip: 1 })
      appendParams({ aStatus: ['s'], nSkip: 1 })
    } else if (name === 'published') {
      setRequestParams({ ...requestParams, aStatus: ['pub'], nSkip: 1 })
      appendParams({ aStatus: ['pub'], nSkip: 1 })
    } else if (name === 'expired') {
      setRequestParams({ ...requestParams, aStatus: ['ex'], nSkip: 1 })
      appendParams({ aStatus: ['ex'], nSkip: 1 })
    }
    changeTab(name)
  }
  ...
  return (
    <>
      <TopBar
        buttons={[
          { text: 'New Poll', icon: 'icon-add', type: 'primary', clickEventName: 'newPoll', isAllowedTo: 'CREATE_POLL' }
        ]}
