...
function TinyEditor({ control, error, register, required, initialValue, values, disabled, name, onlyTextFormatting, setValue, isLiveArticle, togglePoll }) {
  ...
  const handleMediaGalary = (data, tab) => {
    editorRef.current && editorRef.current.insertContent(renderToStaticMarkup(
  ...
  function handleSocialPreview(type) {
    ...
    if (type === 'twitter') {
      fetch(`${ARTICLE_BASE_URL}api/generate-twitter-data/${inputContent}`, {
        ...
        if (data?.message === 'Something went wrong') {
          ...
          data && editorRef.current.insertContent(handleTweet(data))
          setValue('social-url-preview', '')
        }
      })
    } else if (type === 'instagram') {
      const instagramUrl = inputContent.split('?')[0]
      editorRef.current.insertContent(renderToStaticMarkup(
    ...
    return {
      ...
      onSubmit: async (api) => {
        ...
        if (media === 'Tweet' && inputContent?.length > 0) {
          ...
          api.close()
        } else if (media === 'Poll' && inputContent?.length > 0) {
          editor.insertContent(renderToStaticMarkup(
            <div className='ct-poll mceNonEditable' id='ct-poll'></div>
          ))
          setValue('iPollId', inputContent)
          api.close()
        } else {
          dispatch({
            type: 'SHOW_TOAST',
        ...
        if (inputContent?.length > 0) {
          if (values()?.iEventId !== inputContent) {
            if (eventId?.id === 'ct-liveBlog') {
              ...
              editor.insertContent(renderToStaticMarkup(
                <div className='ct-liveBlog mceNonEditable' id='ct-liveBlog'></div>
              ))
              setValue('iEventId', inputContent)
            }
          }
        ...
        render={({ field: { onChange, value } }) => (
          <Editor
            ...
            init={{
              ...
              min_height: isLiveArticle ? 470 : 1000,
              image_caption: true,
              invalid_styles: {
              ...
              setup: (editor) => {
                ...
                editor.ui.registry.addButton('addTweet', {
                  icon: 'twitter',
                  onAction: (_) => {
                    editor.windowManager.open(handleModal(editor, 'Tweet', 'ID'))
                  }
                })
