...
function Publish({
  ...
  function prepareArticleData(value) {
    ...
    data.aPollId = data?.aPolls?.map(poll => poll?._id)
    delete data?.aPolls
  ...
  function mapArticleDataAsPerApi(value) {
    const data = { ...value, oSeo: { ...value.oSeo }, oImg: { ...value.oImg }, oTImg: { ...value.oTImg }, oSticky: { ...value?.oSticky }, aSeries: value?.aSeries || [] }
