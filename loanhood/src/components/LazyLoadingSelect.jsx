...
function LazyLoadingSelect({
  ...
  const [requestParams, setRequestParams] = useState({
    ...
    limit: 10,
    searchValue: '',
    sortField: selectorName === 'allRentals' ? 'title' : '' || storeName === 'users' ? 'firstName' : 'name',
    sortOrder: 1,
    id: categoryId,
    filter: (selectorName === 'allRentals' || 'users') && []
  })

  const firstNameParam = getQueryVariable('firstName')
  const lastNameParam = getQueryVariable('lastName')
  ...
  useEffect(() => {
    if (rentalIdParam) {
      const data = { ...requestParams, offset: 0, filter: [{ rentalId: rentalIdParam }] }
      dispatch(api(data))
    }
  }, [])
  useEffect(() => {
  ...
  function getName() {
    if (!firstNameParam && selectedItem && currentStore === 'users') {
      return selectedItem.firstName + ' ' + selectedItem.lastName
    } else if (selectedItem && currentStore === 'brand') {
      return selectedItem.name
    } else if (selectedItem && currentStore === 'subCategory') {
      return selectedItem.name
    } else if (selectedItem && currentStore === 'sizeGroup') {
      return selectedItem.name
    } else if (selectedItem && currentStore === 'size') {
      return selectedItem.name
    } else if (selectedItem && currentStore === 'material') {
      return selectedItem.name
    } else if (selectedItem && currentStore === 'rental') {
      return selectedItem.title
    } else if (firstNameParam && currentStore === 'users') {
