...
import { Avatar, Box, Button, TableCell, TableRow, Typography } from '@material-ui/core'
import PageTitle from 'components/PageTitle'
import DataTable from 'components/DataTable'
import { GetSplashScreen } from 'state/actions/splashScreen'
...
function SplashScreen() {
  ...
  const [allSplashScreen, setAllSplashScreen] = useState([])
  ...
  const [columnItems, setColumnItems] = useState([
    ...
    { name: 'Text Color', internalName: 'textColor', type: 0 }
  ])
  const splashScreen = useSelector((state) => state.splashScreen.splashScreens)
  ...
  useEffect(() => {
    setIsLoading(true)
    dispatch(GetSplashScreen(requestParams))
  }, [])

  useEffect(() => {
    if (splashScreen && splashScreen.rows) {
      setAllSplashScreen(splashScreen.rows)
      setTotalRecords(splashScreen.count)
      setIsLoading(false)
    }
  }, [splashScreen])
  ...
  function handleAdd() {
    history.push('/splash-screen/add')
  }
  ...
  function goToDetail(id) {
    history.push('/splash-screen/detail/' + id)
  }
  ...
  return (
    <>
      ...
      <PageTitle title="Splash Screen" />
      <DataTable
        columnItems={columnItems}
        isLoading={isLoading}
        searchEvent={(e) => handleSearch(e)}
        addBtnEvent={() => handleAdd()}
        addBtnTxt={allSplashScreen.length === 0 ? 'Add Splash Screen' : ''}
        ...
        {allSplashScreen.map((item) => {
          return (
            <TableRow onClick={() => goToDetail(item.id)} hover key={item.id}>
              ...
              <TableCell>
                <Box component="div" style={{ backgroundColor: item.textColor }} p={1} />
              </TableCell>
