...
function SizeGroups() {
  ...
  const [allSizeGroups, setAllSizeGroups] = useState([])
  ...
  const sizeGroups = useSelector((state) => state.sizeGroup.sizeGroups)
  const resStatus = useSelector((state) => state.sizeGroup.resStatus)
  const resMessage = useSelector((state) => state.sizeGroup.resMessage)

  useEffect(() => {
    setIsLoading(true)
    dispatch(GetSizeGroups(requestParams))
  }, [])

  useEffect(() => {
    if (sizeGroups && sizeGroups.rows) {
      setAllSizeGroups(sizeGroups.rows)
      setTotalRecords(sizeGroups.count)
      setIsLoading(false)
    }
  }, [sizeGroups])
