...
import { GetColors } from 'state/actions/color'
...
function Colors() {
  ...
  useEffect(() => {
    setIsLoading(true)
    dispatch(GetColors(requestParams))
  }, [])

  useEffect(() => {
    if (colors && colors.rows) {
      setAllColors(colors.rows)
      setTotalRecords(colors.count)
      setIsLoading(false)
    }
  }, [colors])
        ...
        {allColors.map((item) => {
          return (
            <TableRow onClick={() => goToDetail(item.id)} hover key={item.id}>
              ...
              <TableCell>
                <Avatar variant="rounded" alt={item.name} src={item.uri.replace('cdn.loanhood.com/', 'ik.imagekit.io/w6paupmag1w/')} />
              </TableCell>
