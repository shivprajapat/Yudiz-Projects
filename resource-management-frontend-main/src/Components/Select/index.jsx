...
import { handleAlterImage } from 'helpers'
...
function Select(
  ...
  const Option = ({children, ...props}) => {
    return (
      <components.Option {...props}>
        {props.isEmployeeSelector ? (<div style={{ display: 'flex' }}>
        <div className="emp-profile">
          <div className="emp-profile-img">
            {props?.data?.sProfilePic ? (
              <img
                src={`https://jr-web-developer.s3.ap-south-1.amazonaws.com/${props?.data?.sProfilePic}`}
              ...
              <AlterImage text={props?.data?.sName} />
            )}
          </div>
        </div>
        <div style={{ marginLeft: '10px' }}>
          <div>{props?.data?.sName}</div>
          <div>
            {props?.data?.sJobProfile?.sPrefix ? props?.data?.sJobProfile?.sPrefix + ' ' + props?.data?.sJobProfile?.sName : props?.data?.sJobProfile?.sName}
          </div>
          <div>{`Emp #${props?.data?.sEmpId} | ${props?.data?.sDepartment?.sName}`}</div>
        </div>
      </div>) : (children)}
      </components.Option>
    )
  ...
  Option.propTypes = {
    ...
    data: PropTypes.shape({
      ...
      sEmpId: PropTypes.string,
      sJobProfile: PropTypes.shape({
        sPrefix: PropTypes.string,
        sName: PropTypes.string
      }),
      sDepartment: PropTypes.shape({
        sName: PropTypes.string
      }),
      sProfilePic: PropTypes.string
    }),
    isEmployeeSelector: PropTypes.bool
  }
    ...
    <>
      <div className={'select-container ' + (className || '')} style={{ width: width || 'unset' }}>
        ...
        <Component
          ...
          components={{ ValueContainer, Option: (innerProps) => (<Option {...innerProps} {...props} />) }}
          options={options}
