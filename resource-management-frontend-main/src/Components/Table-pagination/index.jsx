    ...
    <div className="d-flex justify-content-between pagination-section">
      <div className="d-flex align-items-center">
        <div className="text-muted">
          Page {(currentPage || 0) + 1} Of {lastPage || 1}
        </div>
        <div className="ms-3 p-2 px-3 bg-white text-muted" style={{ borderRadius: 8 }}>
          <Dropdown>
            <Dropdown.Toggle as={CustomToggle} id="dropdown-basic">
              Row Limit: {pageSize === 'all' ? 'All' : pageSize}
            </Dropdown.Toggle>

            <Dropdown.Menu>
              {rowsOptions?.map((i) => (
                <Dropdown.Item as="button" key={i} onClick={() => onLimitChange(i)} className='text-capitalize'>
                  {i}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
      ...
      {totalCount > pageSize && (
        <div className="d-flex align-items-center">
          ...
          <div
            className={'btn bg-white mx-1 py-3 ' + ((currentPage + 1 === lastPage || !totalCount) && 'disabled')}
            onClick={onNext}
          >
            <RightArrow />
          </div>
...
TablePagination.defaultProps = {
  rowsOptions: [5, 10, 15, 20, 25, 50, 100, 'all'],
}
