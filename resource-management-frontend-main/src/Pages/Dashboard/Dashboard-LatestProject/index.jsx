...
//query
import { getLatestProjects } from 'Query/Dashboard/dashboard.query'
...
function DashboardLatestProject() {
    ...
    // get latestProject
    const { data: latestProjectsDetails, isLoading } = useQuery({
        queryKey: ['latestProjects', requestParams],
        queryFn: () => getLatestProjects(requestParams),
        select: (data) => data.data.data,
        staleTime: 10000,
        enabled: isGranted(permissionsName.VIEW_DASHBOARD_LATEST_PROJECTS)
    })
    ...
    return (
        <Wrapper
            ...
            <div className='d-flex justify-content-between'>
                ...
                />
                {
                    latestProjectsDetails?.eShowAllProjects === 'ALL' ?
                        <Select
            ...
            <DataTable
                columns={[
                    ...
                    { name: 'Start Date', connectionName: 'sDepartment', isSorting: false, sort: 0 },
                ]}
                disableActions
                totalData={latestProjectsDetails?.data?.length}
            >
                {
                    latestProjectsDetails?.data?.map((d, index) => {
                        return (
                            <tr key={index}
...
export default DashboardLatestProject