...
import { getCountries } from 'Query/Country/country'
import { getFreeResource, getLatestProjects, getMonthlyProjects } from 'Query/Dashboard/dashboard.query'
import { departmentList, getEmployeeList, jobProfileList, roleList, permissionList, orgBranchList } from 'Query/Employee/employee.query'
import { TechnologyList } from 'Query/Interview/interviews.query'
import { getProjectList, getProjectTags, getBA, getBDE, getPM } from 'Query/Project/project.query'
...
export default function useResourceDetails(queryKeys) {
  ...
  const allQueryConfig = [
    ...
    {
      q_key: 'orgbranch',
      queryKey: ['resource_orgbranch', pages?.orgbranch, searches?.orgbranch],
      queryFn: () => orgBranchList(lazyLoad?.current?.orgbranch),
      select: (data) => data.data.data.branch,
      onSuccess: function (data) {
        handleSettingData(data, this.q_key)
      },
      refetchOnMount: true,
    ...
    },
    {
      ...
      queryFn: () => getBA(lazyLoad?.current?.ba),
    ...
    },
    {
      ...
      queryFn: () => getBDE(lazyLoad?.current?.bde),
      select: (data) => data.data.data.data,
      onSuccess: function (data) {
        handleSettingData(data, this.q_key)
      },
    ...
    },
    {
      ...
      queryFn: () => getPM(lazyLoad.current?.projectManager),
      select: (data) => data.data.data.data,
      onSuccess: function (data) {
        handleSettingData(data, this.q_key)
      },
    ...
    {
      q_key: 'country',
      queryKey: ['resource_country', pages?.country, searches?.country],
      queryFn: () => getCountries(lazyLoad?.current?.country),
      select: (data) => data.data.data.country,
      onSuccess: function (data) {
        handleSettingData(data, this.q_key)
      },
      refetchOnMount: true,
