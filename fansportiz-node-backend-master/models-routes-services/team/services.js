...
class Team {
  ...
  async addTeam(req, res) {
    try {
      ...
      req.body = pick(req.body, ['sImage', 'sKey', 'sName', 'sShortName', 'eStatus'])
  ...
  async update(req, res) {
    try {
      const { sKey, sportsType, sImage, eStatus } = req.body
      const eCategory = sportsType.toUpperCase()

      req.body = pick(req.body, ['sName', 'sKey', 'sShortName', 'eStatus'])
      ...
      const team = await TeamModel.findByIdAndUpdate(req.params.id, { ...req.body, eCategory, sImage, dUpdatedAt: Date.now() }, { new: true, runValidators: true }).lean()
      ...
      if (team && oldTeam) {
        const { sName, sShortName, sImage } = team
  ...
  async getSignedUrl(req, res) {
    try {
      ...
      const data = await bucket.getSignedUrl({ sFileName, sContentType, path: config.s3Teams })
