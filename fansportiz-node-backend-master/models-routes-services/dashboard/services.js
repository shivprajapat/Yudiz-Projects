...
const { getDepositOfDateRange, getDateRangeWithdrawOfPG } = require('../report/grpc/clientServices')
      ...
      try {
        ...
        for (const date of aDateRange) {
          ...
          if (['RUDW', 'RUMW', 'RUYW', 'UTDW', 'UTMW', 'UTYW'].includes(sKey)) {
            ...
            const nTotalSuccess = await getDepositOfDateRange({ where: { ePaymentStatus: 'S' }, raw: true }, { dStartDate: new Date(Number(dStart)), dEndDate: new Date(Number(dEnd)) })
            nValue = nTotalSuccess.length ? (!nTotalSuccess[0].total ? 0 : nTotalSuccess[0].total) : 0
          } else if (['WDW', 'WMW', 'WYW'].includes(sKey)) {
            const nTotalSuccess = await getDateRangeWithdrawOfPG({ group: 'ePaymentGateway', where: { ePaymentStatus: 'S' }, raw: true }, { dStartDate: new Date(Number(dStart)), dEndDate: new Date(Number(dEnd)) })
            nValue = nTotalSuccess.length ? nTotalSuccess.reduce((acc, { nValue }) => acc + nValue, 0) : 0
          }
