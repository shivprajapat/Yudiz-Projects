...
class ApiLog {
  ...
  async listTransactionLog(req, res) {
    try {
      ...
      // For Deposit list we are considering iReferenceId. Make sure FE will send same
      const query = (eType === 'W') ? { iWithdrawId: Number(req.params.id) } : { iReferenceId: req.params.id }
