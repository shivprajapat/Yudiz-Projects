  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        oPrivate.nAdminCommission = leagueCom.length ? convertToDecimal(leagueCom[0].nAdminCommission) : 0
        oPrivate.nCreatorCommission = leagueCom.length ? convertToDecimal(leagueCom[0].nCreatorCommission) : 0

        const amount = await calculateTurnOver([iMatchId], true)
        const userAmount = amount.filter(({ _id }) => _id === 'U')

        oPrivate.nAmount = userAmount.length ? convertToDecimal(userAmount[0].nAmount) : 0
        oPrivate.nBonus = 0
        oPrivate.nCash = userAmount.length ? convertToDecimal(userAmount[0].nAmount) : 0
        oPrivate.nDistAmount = userAmount.length ? convertToDecimal(userAmount[0].nDistAmount) : 0
        oPrivate.nDistBonus = userAmount.length ? convertToDecimal(userAmount[0].nDistBonus) : 0
        ...
        oPublic.nAmount = userAmountPublic.length ? convertToDecimal(userAmountPublic[0].nAmount) : 0
        oPublic.nBotAmount = botAmount.length ? convertToDecimal(botAmount[0].nAmount) : 0

        oPublic.nDistAmount = userAmountPublic.length ? convertToDecimal(userAmountPublic[0].nDistAmount) : 0
        oPublic.nDistBonus = userAmountPublic.length ? convertToDecimal(userAmountPublic[0].nDistBonus) : 0
        oPublic.nBotDistAmount = botAmount.length ? convertToDecimal(botAmount[0].nDistAmount) : 0
        oPublic.nBotDistBonus = botAmount.length ? convertToDecimal(botAmount[0].nDistBonus) : 0

        oPublic.nPromoDiscount = userAmountPublic.length ? convertToDecimal(userAmountPublic[0].nPromoDiscount) : 0
        oPublic.nBotPromoDiscount = botAmount.length ? convertToDecimal(botAmount[0].nPromoDiscount) : 0

        oPublic.nAmount = convertToDecimal(oPublic.nAmount - oPublic.nPromoDiscount)

        oPublic.nBonus = await calculatePublicLeagueBonus([iMatchId], 'U')
        oPublic.nBotBonus = await calculatePublicLeagueBonus([iMatchId], 'B')

        oPublic.nCash = convertToDecimal(oPublic.nAmount - oPublic.nBonus)
        oPublic.nBotCash = convertToDecimal(oPublic.nBotAmount - oPublic.nBotBonus)
        ...
        for (const topTenSpender of topTenSpendUser) {
          ...
          const aUserLeagueIds = matchLeagues.map((e) => e.iUserLeagueId.toString())
          const [nTeams, passbooks, users] = await Promise.all([
            UserTeamModel.countDocuments({ iMatchId, iUserId }),
            PassbookModel.findAll({ where: { iUserId: iUserId.toString(), iUserLeagueId: { [Op.in]: aUserLeagueIds } }, raw: true }),
            UsersModel.findById(iUserId, { sUsername: 1, sMobNum: 1, sEmail: 1, eType: 1, _id: 0 }).lean()
          ])

          const nBonusUtil = passbooks.reduce((acc, element) => {
            acc += element.nBonus
            return acc
          }, 0)
      ...
      try {
        ...
        for (const matchLeague of matchleagues) {
          ...
          if (userLeagues.length) {
            const aUserIds = userLeagues.map((e) => e.iUserId.toString())
            const aUserLeagueIds = userLeagues.map((e) => e.iUserLeagueId.toString())

            const passbooks = await PassbookModel.findAll({ where: { iUserId: { [Op.in]: aUserIds }, iUserLeagueId: { [Op.in]: aUserLeagueIds }, iMatchLeagueId: _id.toString(), eTransactionType: 'Play' }, raw: true })

            nBonus = passbooks.reduce((acc, element) => {
              acc += element.nBonus
              return acc
            }, 0)
          }
