...
const { findTotalCashDeposit, getDepositOfPG, getDepositOfDateRange, getDepositOfPgInDateRange, getWithdrawOfPG, getDateRangeWithdrawOfPG } = require('./grpc/clientServices')
// eType change
class Report {
  ...
  async fetchUserReport(req, res) {
    try {
      ...
      if (eKey === 'TU') {
        ...
        const [nTotalDeposits, nTotalPendingDeposits, nTotalSuccessDeposits, nTotalCancelledDeposits, nTotalRejectedDeposits, aDeposits] = await Promise.all([
        ...
        const [aSuccessWithdrawals, aPendingWithdrawals] = await Promise.all([
...
function getDates () {
  ...
  const weekEnd = new Date(weekStart + 6)
