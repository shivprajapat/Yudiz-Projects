...
describe('MatchLeague routes', () => {
  ...
  after(() => {
    globalStore.matchLeagueID = store.iMatchLeagueId
    globalStore.leagueID = store.leagueID
  })
  ...
  describe('/POST MatchLeague add', () => {
    ...
    it('Should not add MatchLeague because invalid match id', (done) => {
      request(server)
        .post('/api/admin/match-league/v1')
        .set('Authorization', store.token)
        .send({
    ...
    it('Should not add MatchLeague because invalid league ids', (done) => {
      request(server)
        .post('/api/admin/match-league/v1')
        .set('Authorization', store.token)
        .send({
          iMatchId: store.iMatchId,
          iLeagueId: [{ _id: store.wID }]
        })
        .expect(status.OK)
        .end(function(err, res) {
          if (err) return done(err)
          expect(res.body.message).toMatch(messages.English.invalid.replace('##', messages.English.cleagues))
          done()
        })
    })
  })
    ...
    it('Should be get League list', (done) => {
      request(server)
        ...
        .end((err, res) => {
          ...
          store.leagueID = res.body.data[0].results[0].iLeagueId
          done()
        })
    })
  })
  ...
  describe('/GET MatchLeague ', () => {
    ...
    it('Should not be get single MatchLeague', (done) => {
      request(server)
        .get(`/api/admin/match-league/${store.wID}/v1`)
        .set('Authorization', store.token)
        .expect(status.NotFound)
        .end(function(err, res) {
          if (err) return done(err)
  ...
  describe('/GET Admin: match league report', () => {
    it('Should be get match league report', (done) => {
      request(server)
        .get(`/api/admin/match-league/${store.iMatchId}/report/v1`)
        .set('Authorization', store.token)
        .expect(status.OK)
        .end(function(err, res) {
          if (err) return done(err)
