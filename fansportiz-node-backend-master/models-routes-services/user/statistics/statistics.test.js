...
describe('User Statistics Routes', () => {
  ...
  describe('/GET Admin: List of users statistics', () => {
    ...
    it('Should not be fetch statistics list because invalid id', (done) => {
      request(server)
        .get(`/api/admin/statistics/${store.wId}/v1`)
        .set('Authorization', store.token)
  ...
  describe('/GET user profile statistics', () => {
    it('Should be fetch user profile statistics', (done) => {
      request(server)
        .get(`/api/user/profile-statistics/${store.ID}/v1`)
        .expect(status.OK)
        .end(function(err, res) {
          if (err) return done(err)
