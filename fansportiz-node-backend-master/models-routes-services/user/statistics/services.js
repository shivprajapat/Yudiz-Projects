...
const { bAllowDiskUse, DELETED_USERNAME } = require('../../../config/config')
class Statistic {
  async get(req, res) {
    try {
  ...
  async getLeaderShipboardV2(req, res) {
    try {
      const populateFields = req.admin ? ['sName', 'sProPic', 'sUsername', 'eStatus', 'eType'] : ['sName', 'sProPic', 'eStatus', 'sUsername']
      ...
      if (dataRes) {
        if (dataRes.oMonthData && dataRes.oMonthData.aData && dataRes.oMonthData.aData.length) {
          dataRes.oMonthData.aData.forEach((d) => {
            ...
            if (d.oUser.eStatus === 'D') {
              d.oUser.sUsername = DELETED_USERNAME
            }
            delete d.iUserId
            delete d.oUser.eStatus
          })
        }
  ...
  async getProfileStatistics(req, res) {
    try {
      const [statistics, aMatchData] = await Promise.all([
        StatisticsModel.findOne({ iUserId: ObjectId(req.params.id) }, { nTotalJoinLeague: 1, oCricket: 1, oBaseball: 1, oFootball: 1, oBasketball: 1, oKabaddi: 1, iUserId: 1 })
          .populate('iUserId', ['nLoyaltyPoints', 'dCreatedAt', 'sUsername', 'sProPic']).lean(),
        MyMatchesModel.aggregate([
          { $match: { iUserId: ObjectId(req.params.id), $and: [{ aMatchLeagueId: { $exists: true } }, { 'aMatchLeagueId.0': { $exists: true } }] } },
          { $group: { _id: '$iSeasonId', total: { $sum: 1 } } },
          { $group: { _id: null, nTotalSeries: { $sum: 1 }, nTotalMatches: { $sum: '$total' } } }
        ]).allowDiskUse(bAllowDiskUse)
      ])
      const statisticsCounts = { ...statistics.iUserId, nTotalJoinLeague: statistics.nTotalJoinLeague, nCricket: statistics.oCricket.nJoinLeague, nBaseball: statistics.oBaseball.nJoinLeague, nFootball: statistics.oFootball.nJoinLeague, nBasketball: statistics.oBasketball.nJoinLeague, nKabaddi: statistics.oKabaddi.nJoinLeague }
      const nTotalSeries = aMatchData && aMatchData[0] && aMatchData[0].nTotalSeries ? aMatchData[0].nTotalSeries : 0
      const nTotalMatch = aMatchData && aMatchData[0] && aMatchData[0].nTotalSeries ? aMatchData[0].nTotalMatches : 0

      return res.status(status.OK).jsonp({ status: jsonStatus.OK, message: messages[req.userLanguage].success.replace('##', messages[req.userLanguage].user), data: { ...statisticsCounts, nTotalMatch, nTotalSeries } })
    } catch (error) {
