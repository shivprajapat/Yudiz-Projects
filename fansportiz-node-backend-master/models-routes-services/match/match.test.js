...
const { getToken, globalStore } = require('../../config/testStore')
...
describe('Match routes', () => {
  before(async () => {
    ...
    store.token = token.adminToken
    store.wId = '62e2541f479a02b559b42450'
    store.userData = token.userToken.data
    store.userToken = token.userToken.Authorization
    store.iUserId = token.userToken.data._id
    store.sEmail = token.userToken.data.sEmail
    store.sName = token.userToken.data.sName
  })

  after(() => {
    globalStore.adminToken = store.token
    globalStore.userToken = store.userToken
    globalStore.userEmail = store.sEmail
    globalStore.sName = store.sName
    globalStore.userData = store.userData
    // globalStore.matchID = store.entityMatchId
    globalStore.userID = store.iUserId
    globalStore.HomeID = store.entityMatchData.oHomeTeam.iTeamId
    globalStore.AwayID = store.entityMatchData.oAwayTeam.iTeamId
    globalStore.MatchData = store.entityMatchData
    globalStore.CustomMatchId = store.ID
    globalStore.CustomHomeId = store.HomeID
  })
  ...
  describe('/GET match list', () => {
    it('Should be get Match list', (done) => {
      request(server)
        .get('/api/admin/match/list/v1?start=0&limit=10&order=des&sportsType=cricket')
        .set('Authorization', store.token)
        .expect(status.OK)
        .end((err, res) => {
          if (err) return done(err)
    ...
    it('Should be get Match list of entitySports', (done) => {
      ...
      request(server)
        .get(`/api/admin/match/list/v1?start=0&limit=10&sort=dCreatedAt&order=desc&datefrom=${dateFrom}&dateto=${dateTo}&sportsType=cricket&eProvider=ENTITYSPORT`)
        .set('Authorization', store.token)
        .expect(status.OK)
        .end((err, res) => {
          if (err) return done(err)
          expect(res.body.message).toMatch(messages.English.success.replace('##', messages.English.match))
          store.entityMatchId = res.body.data[0].results[1]._id
          globalStore.matchID = store.entityMatchId
          store.entityMatchData = res.body.data[0].results[1]
          done()
        })
    })
  ...
  describe('/PUT update match details', () => {
    ...
    it('Should be updated match details entity sport match', (done) => {
      const tomorrow = new Date()
      tomorrow.setDate(tomorrow.getDate() + 7)
      const data = { ...store.entityMatchData, iHomeTeamId: store.entityMatchData.oHomeTeam.iTeamId, iAwayTeamId: store.entityMatchData.oAwayTeam.iTeamId, eStatus: 'U', dStartDate: tomorrow }

      request(server)
        .put(`/api/admin/match/${store.entityMatchId}/v1`)
        .set('Authorization', store.token)
        .send(data)
        .expect(status.OK)
        .end(function (err, res) {
    ...
    it('Should not be update match details because invalid away id', (done) => {
      request(server)
        .put(`/api/admin/match/${store.ID}/v1`)
        .set('Authorization', store.token)
        .send({
    ...
    it('Should not be update match details because invalid season id', (done) => {
      request(server)
        .put(`/api/admin/match/${store.ID}/v1`)
        .set('Authorization', store.token)
        .send({
          ...
          iHomeTeamId: `${store.HomeID}`,
          iAwayTeamId: `${store.awayID}`,
          iSeasonId: '5f7f0fd9b18344309eb41008'
        })
        .expect(status.NotFound)
        .end((err, res) => {
          if (err) return done(err)
    ...
    it('Should not be update match details because invalid series id', (done) => {
      request(server)
        .put(`/api/admin/match/${store.ID}/v1`)
        .set('Authorization', store.token)
        .send({
          ...
          iHomeTeamId: `${store.HomeID}`,
          iAwayTeamId: `${store.awayID}`,
          iSeriesId: '5f7f0fd9b18344309eb41008'
        })
        .expect(status.NotFound)
        .end((err, res) => {
          if (err) return done(err)
  ...
  describe('/POST refresh match detail using third party api', () => {
    it('Should be refresh match detail', (done) => {
      request(server)
        .post(`/api/admin/match/refresh/${store.entityMatchId}/v1`)
        .set('Authorization', store.token)
        .expect(status.OK)
        .end(function (err, res) {
          if (err) return done(err)
