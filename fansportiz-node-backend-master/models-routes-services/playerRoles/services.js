...
class PlayerRole {
  ...
  async getMatchPlayerRole(req, res) {
    try {
      const data = await MatchModel.findOne({ _id: ObjectId(req.params.iMatchId) }, { aPlayerRole: 1 }).lean()
      ...
      return res.status(status.OK).jsonp({ status: jsonStatus.OK, message: messages[req.userLanguage].success.replace('##', messages[req.userLanguage].cplayerRole), data: data.aPlayerRole })
    } catch (error) {
      catchError('PlayerRole.getMatchPlayerRole', error, req, res)
    }
  }
}
