...
class Sport {
  ...
  async adminActiveSportsV2(req, res) {
    try {
      const data = await SportModel.find({ eStatus: 'Y' }, { sName: 1, _id: 0, sKey: 1 }).lean()
      return res.status(status.OK).jsonp({ status: jsonStatus.OK, message: messages[req.userLanguage].success.replace('##', messages[req.userLanguage].cactiveSports), data })
    } catch (error) {
      return catchError('Sport.activeSports', error, req, res)
    }
  ...
  async activeSportsV2(req, res) {
    try {
      let data = await SportModel.find({ eStatus: 'Y' }, { __v: 0, dCreatedAt: 0, dUpdatedAt: 0, eStatus: 0, _id: 0, sExternalId: 0 }).populate({ path: 'aPlayerRoles', select: ['aRole.eKey', 'aRole.sName', 'aRole.nPosition'] }).lean()
      data = data.map(data => { return { ...data, aPlayerRoles: data.aPlayerRoles && data.aPlayerRoles.aRole ? data.aPlayerRoles.aRole : [] } })
