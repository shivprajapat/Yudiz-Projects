...
const { catchError, pick, removenull, getPaginationValues2, checkValidImageType, getBucketName } = require('../../helper/utilities.services')
...
class SeriesLeaderBoard {
  async add(req, res) {
    try {
      ...
      const logData = { oOldFields: {}, oNewFields: data, iAdminId: ObjectId(req.admin._id), eKey: 'SLB', sIP: getIp(req) }
  ...
  async update(req, res) {
    try {
      ...
      const oldData = await SeriesLeaderBoardModel.find({ _id: req.params.id }).lean()
      ...
      const logData = { oOldFields: oldData, oNewFields: data, iAdminId: req.admin._id, eKey: 'SLB', sIP: getIp(req) }
      await createAdminLog(logData)
  ...
  async remove(req, res) {
    try {
      ...
      if (!data) return res.status(status.NotFound).jsonp({ status: jsonStatus.NotFound, message: messages[req.userLanguage].not_exist.replace('##', messages[req.userLanguage].cseries) })
      const logData = { oOldFields: data, oNewFields: { ...data, eActive: 'N' }, iAdminId: req.admin._id, eKey: 'SLB', sIP: getIp(req) }
      await createAdminLog(logData)
  ...
  async addSeriesPriceBreakup(req, res) {
    try {
      ...
      const logData = { oOldFields: {}, oNewFields: data, iAdminId: req.admin._id, eKey: 'SLB', sIP: getIp(req) }
      await createAdminLog(logData)
  ...
  async updatePriceBreakup(req, res) {
    try {
      ...
      const logData = { oOldFields: { sName: seriesCategory.sName, eCategory: seriesCategory.eCategory, ...oldData }, oNewFields: { _id: ObjectId(oldData._id), sName: seriesCategory.sName, eCategory: seriesCategory.eCategory, nRankFrom, nRankTo, nPrize, eRankType }, iAdminId: req.admin._id, eKey: 'SLB', sIP: getIp(req) }
      await createAdminLog(logData)
  ...
  async updateCategory(req, res) {
    try {
      ...
      removenull(req.body)
      const oldData = await SeriesLeaderBoardModel.findOne({ aSeriesCategory: { $elemMatch: { _id: ObjectId(req.params.id) } } }).lean()
      const updateObj = {
  ...
  async removeCategory(req, res) {
    try {
      ...
      const logData = { oOldFields: series, oNewFields: { ...series, eActive: 'N' }, iAdminId: req.admin._id, eKey: 'SLB', sIP: getIp(req) }
      await createAdminLog(logData)
  ...
  async getSignedUrlCategory(req, res) {
    try {
      ...
      const data = await bucket.getSignedUrl({ sFileName, sContentType, path: config.s3SeriesCategories })
    ...
    try {
      ...
      for (const aSeriesCategory of allSeriesCategory) {
        ...
        if (userLeagues.length) {
          await SeriesLBUserRankModel.deleteMany({ iCategoryId: aSeriesCategory._id })
          const data = await generateRank(userLeagues, aSeriesCategory, series._id)
      ...
      for (const aSeriesCategory of allSeriesCategory) {
        ...
        if (aSeriesCategory.eType === 'LOYALTY_DEPOSIT_POINTS') {
          ...
          const aUserDepositEntries = await PassbookModel.findAll({
            ...
            where: { eTransactionType: 'Deposit', [Op.and]: [{ dCreatedAt: { [Op.gte]: new Date('2023-03-31T00:00:00.000Z') } }, { dCreatedAt: { [Op.lte]: new Date('2023-05-28T00:00:00.000Z') } }], eUserType: 'U' },
            limit: aSeriesCategory.nMaxRank,
            raw: true
          })
          ...
          const aUserLoyaltyPoints = await PassbookModel.findAll({
            ...
            group: 'iUserId',
            order: [[col('nUserScore'), 'desc']],
            where: { eTransactionType: 'Loyalty-Point', [Op.and]: [{ dCreatedAt: { [Op.gte]: new Date('2023-03-31T00:00:00.000Z') } }, { dCreatedAt: { [Op.lte]: new Date('2023-05-28T00:00:00.000Z') } }], iMatchLeagueId: { [Op.in]: matchLeagueId }, eUserType: 'U' },
            limit: aSeriesCategory.nMaxRank,
            raw: true
          })
        ...
        await queuePush('AdminLogs', logData)
      }
      const logData = { eKey: 'SLB', iAdminId: ObjectId(iAdminId), sIP: getIp(req) }
      await queuePush('AdminLogs', logData)
      return res.status(status.OK).jsonp({ status: jsonStatus.OK, message: messages[req.userLanguage].success.replace('##', messages[req.userLanguage].crankCalculate) })
  ...
  async getAllRankV2(req, res) {
    try {
      ...
      if (req.admin) {
        ...
        if (!data) return res.status(status.NotFound).jsonp({ status: jsonStatus.NotFound, message: messages[req.userLanguage].series_pending })
  ...
  async winDistributionBySeriesCategory(req, res) {
    try {
      const data = await SeriesLeaderBoardModel.findOne({
        _id: ObjectId(req.params.id),
        eStatus: 'CMP'
      }, { aSeriesCategory: 1, _id: 1, sName: 1, eCategory: 1 }).lean()
      if (!data || data.bPriceDone) return res.status(status.NotFound).jsonp({ status: jsonStatus.NotFound, message: messages[req.userLanguage].cseriesAlreadyDistributeAndNotExist.replace('##', messages[req.userLanguage].cseries) })
