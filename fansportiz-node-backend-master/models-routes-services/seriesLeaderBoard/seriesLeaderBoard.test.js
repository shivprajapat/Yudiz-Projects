...
describe('Series LeaderBoard Routes', () => {
  ...
  describe('/GET get single series', () => {
    ...
    it('Should not be get series', (done) => {
      request(server)
        .get(`/api/admin/series-leaderboard/${store.wID}/v1`)
        .set('Authorization', store.adminToken)
  ...
  describe('/PUT update single series', () => {
    ...
    it('Should not be update series', (done) => {
      request(server)
        .put(`/api/admin/series-leaderboard/${store.wID}/v1`)
        .set('Authorization', store.adminToken)
        .send({
          sName: 'TEST-CASES 2021',
          sInfo: 'info about series',
...
describe('Series LeaderBoard Category Routes', () => {
  ...
  describe('/POST get pre signed url ', () => {
    ...
    it('Should not be get pre signed url because invalid content type', (done) => {
      const data = {
        sFileName: 'test.jpg',
  ...
  describe('/POST add series leaderboard category', () => {
    ...
    it('Should not be add series leaderboard category because invalid series leaderboard id', (done) => {
      const data = {
        sName: 'TEST', iCategoryId: store.iCategoryId, sFirstPrize: 100, nMaxRank: 10, sContent: 'TEST', nTotalPayout: 1000
      }
      request(server)
  ...
  describe('/POST add series leaderboard category price breakup', () => {
    ...
    it('Should not be add series leaderboard category price breakup because info not added', (done) => {
      const data = {
        nRankFrom: 1, nRankTo: 1, eRankType: 'E', nPrize: 100
      }
      request(server)
        .post(`/api/admin/series-leaderboard-category/${store.iSeriesCategoryId}/price-breakup/v1`)
        .set('Authorization', store.adminToken)
        ...
        .end(function(err, res) {
          if (err) return done(err)
          expect(res.body.message).toMatch(messages.English.invalid.replace('##', messages.English.ssinfo))
          done()
        })
    })

    it('Should not be add series leaderboard category price breakup because prize is invalid', (done) => {
      const data = {
        nRankFrom: 1, nRankTo: 1, eRankType: 'E', nPrize: 110, sInfo: 'test info'
      }
      request(server)
        .post(`/api/admin/series-leaderboard-category/${store.iSeriesCategoryId}/price-breakup/v1`)
        .set('Authorization', store.adminToken)
    ...
    it('Should not be add series leaderboard category price breakup because rankFrom is greater than rankTo', (done) => {
      const data = {
        nRankFrom: 2, nRankTo: 1, eRankType: 'R', nPrize: 100
      }
      request(server)
        .post(`/api/admin/series-leaderboard-category/${store.iSeriesCategoryId}/price-breakup/v1`)
        .set('Authorization', store.adminToken)
        ...
        .end(function(err, res) {
          if (err) return done(err)
          expect(res.body.message).toMatch(messages.English.less_then_err.replace('##', messages.English.crankFrom).replace('#', messages.English.crankTo))
          done()
        })
    })

    it('Should not be add series leaderboard category price breakup because rank price exists', (done) => {
      const data = {
        nRankFrom: 1, nRankTo: 1, eRankType: 'R', nPrize: 100
      }
      request(server)
