...
function Feedbacks({ userPermission }) {
  ...
  const [tabs, setTabs] = useState([
    { name: <FormattedMessage id="editorialSuggestion" />, internalName: 'editorialSuggestion', active: true },
    { name: <FormattedMessage id="otherEnquiries" />, internalName: 'otherEnquiries', active: false }
  ])
  const labels = {
  ...
  const { loading } = useQuery(GET_FEEDBACKS_LIST, {
  ...
  function checkTab() {
    checkTagParam().activeTabName !== 'EditorialSuggestion' && changeTab(checkTagParam().activeTabName)
  }
  ...
  function checkTagParam() {
    if (requestParams.eQueryType === 'e') {
      return {
        params: 'e',
        activeTabName: 'EditorialSuggestion'
      }
    } else if (requestParams.eQueryType === 's') {
      return { params: 's', activeTabName: 'otherEnquiries' }
    }
  }
  return (
    <>
