...
function JobPost({ userPermission }) {
  ...
  const [selectedTab, setSelectedTab] = useState('jobPost')
  const [columns, setColumns] = useState(getActionColumns(selectedTab))
  const [tabs, setTabs] = useState([
    { name: <FormattedMessage id="jobPost" />, internalName: 'jobPost', active: true, isAllowedTo: 'LIST_PLAYER_TAGS' },
    { name: <FormattedMessage id="enquiriesReceived" />, internalName: 'enquiriesReceived', active: false, isAllowedTo: 'LIST_PLAYER_TAGS' }
  ])
  ...
  function handleTabChange(name) {
    if (name === 'jobPost') {
      setRequestParams({ ...requestParams, nSkip: 1, nLimit: 10, sSearch: '', nOrder: -1, sSortBy: 'dCreated' })
  ...
  function getBulkActionItem(name) {
    if (name === 'jobPost') {
      actionPermission.current = ['EDIT_JOB', 'DELETE_JOB']
      return [
      ...
      actionPermission.current = ['EDIT_JOB']
      return [{ label: <FormattedMessage id="deleteAll" />, value: 'd', isAllowedTo: 'DELETE_JOB' }]
    }
  }
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            if (selectedTab === 'jobPost') {
              ...
              const { data } = await bulkActionEnquiries({ variables: { input: { aId: [id], eStatus: 'd' } } })
              if (data && data.bulkEnquiryUpdate) handleBulkResponse([id], 'd')
            }
    ...
    <>
      ...
      >
        ...
        <Drawer isOpen={isFilterOpen} onClose={() => setIsFilterOpen(!isFilterOpen)} title={useIntl().formatMessage({ id: 'filterJob' })}>
          <JobPostFilters filterChange={handleFilterChange} type="player" />
        </Drawer>
      </DataTable>
    </>
  )
