...
import { GET_COMMENTS, GET_COMMENTS_COUNT } from 'graph-ql/comments/query'
...
import { STATUS_COMMENT, BULK_STATUS_COMMENT } from 'graph-ql/comments/mutation'
...
function ArticleComments() {
  ...
  const { loading } = useQuery(GET_COMMENTS, {
    ...
    onCompleted: (data) => {
      if (data && data?.listComments.aResults) {
        setSelectedComment(
          data.listComments.aResults.map((item) => {
            return {
              _id: item._id,
              value: false
            }
        ...
        setCommentList(data.listComments.aResults)
      }
    }
  })

  const [getCommentCount, { commentCountLoading }] = useLazyQuery(GET_COMMENTS_COUNT, {
    onCompleted: (data) => {
      if (data && data?.getCommentCounts) {
        commentCount.current = { ...data.getCommentCounts }
        setTabs(
          tabs.map((e) => {
  ...
  const [statusAction, { loading: statusLoading }] = useMutation(STATUS_COMMENT, {
    onCompleted: (data) => {
      if (data && data.updateCommentStatus) {
        dispatch({
          type: 'SHOW_TOAST',
          payload: { message: data.updateCommentStatus.sMessage, type: TOAST_TYPE.Success, btnTxt: labels.close }
        })
      }
    }
  })

  const [bulkAction, { loading: bulkStatusLoading }] = useMutation(BULK_STATUS_COMMENT, {
    onCompleted: (data) => {
      if (data && data.bulkCommentUpdate) {
        dispatch({
          type: 'SHOW_TOAST',
          payload: { message: data.bulkCommentUpdate.sMessage, type: TOAST_TYPE.Success, btnTxt: labels.close }
        })
      }
    }
  })
