...
import { FANTASY_TIPS_STATUS, DELETE_FANTASY_ARTICLE, CHANGE_FANTASY_ARTICLE_STATUS } from 'graph-ql/fantasy-tips/mutation'
...
const FantasyTipsList = () => {
  ...
  function handleTabChange(name) {
    if (name === 'upcomingMatches') {
      setRequestParams({ ...requestParams, aStatus: ['scheduled', 'live'], eType: 'm', nSkip: 1 })
      appendParams({ aStatus: ['scheduled', 'live'], eType: 'm', nSkip: 1 })
    } else if (name === 'completedMatches') {
      setRequestParams({ ...requestParams, aStatus: ['completed'], eType: 'm', nSkip: 1 })
      appendParams({ aStatus: ['completed'], eType: 'm', nSkip: 1 })
    } else if (name === 'trash') {
      setRequestParams({ ...requestParams, aStatus: ['scheduled', 'live', 'completed', 'cancelled'], eType: 't', nSkip: 1 })
      appendParams({ aStatus: ['scheduled', 'live', 'completed', 'cancelled'], eType: 't', nSkip: 1 })
    }
    changeTab(name)
  }
  ...
  return (
    <>
      ...
      <DataTable
        ...
        isLoading={loading || statusLoading || countLoading}
        header={{
          left: {
            rows: true
          },
