...
function EditTeamView() {
  ...
  function onEditTeam(formValue) {
    const { value, data } = getPreSignedData(formValue, 'team')
      ...
      <Form onSubmit={handleSubmit(onEditTeam)}>
        <Row>
          ...
          <div className="btn-bottom add-border mt-4 ">
            ...
            <Button variant="primary" type="submit" className="m-2" disabled={teamLoading || seoLoading || imgLoading || isImgUploading}>
              <FormattedMessage id={id ? 'update' : 'add'} />
              {(teamLoading || seoLoading || imgLoading || isImgUploading) && <Spinner animation="border" size="sm" />}
            </Button>
          </div>
        </Row>
      </Form>
