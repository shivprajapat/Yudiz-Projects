...
const FeatureImageSidebar = ({ data, isPlugin }) => {
      ...
      buttons: [
        {
          label: labels.yes,
          onClick: async () => {
            await deleteImage({ variables: { input: { _id: imageData?._id } } })
          }
        },
        {
          label: labels.no
  ...
  return (
    <>
      ...
      <div className="feature-image-div">
        ...
        <Row>
          <Col sm={isPlugin ? 5 : 4}>
            <img className="sidebar-image" src={data?.sUrl && S3_PREFIX + data?.sUrl} />
          </Col>
          <Col>
            <span className="media-img-title">{data?.sUrl?.split('/').pop()}</span>
            <br />
            ...
            <span className="dark-text">{convertDateTimeAMPM(data?.dCreated)}</span>
            <br />
            {data?.oMeta?.nSize && (
              <>
