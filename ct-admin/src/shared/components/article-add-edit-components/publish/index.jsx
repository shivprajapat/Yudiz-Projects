...
function Publish({
  ...
  function mapArticleDataAsPerApi(value) {
    const data = { ...value, oSeo: { ...value.oSeo }, oImg: { ...value.oImg }, oTImg: { ...value.oTImg } }
  ...
  return (
    <ArticleTab title="Publish" event={0}>
      ...
      {(isImgUploading || pickLoading || editLoading || loading || seoLoading || getLoading || imgLoading) && <Loading />}
      <RedirectionPopup
