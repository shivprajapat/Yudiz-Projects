...
function CommonSEO({
  ...
  const [generateSlugs] = useLazyQuery(GENERATE_SLUG, {
    onCompleted: (data) => {
      if (data.generateSlugs?.oData) {
        if (data.generateSlugs.oData.sSlug) {
        ...
        if (
          ((data.generateSlugs.oData.sSlug && slugType === 't') || (data.generateSlugs.oData.sSlug && slugType === 'p')) &&
          !values.oSeo.sSlug
        ) {
          setValue('oSeo.sSlug', data.generateSlugs.oData.sSlug)
          extraSlug && setValue(extraSlug, data.generateSlugs.oData.sSlug)
          clearErrors('oSeo.sSlug')
          setAvailableSlug('')
        }
        if (data.generateSlugs.oData.sSlug && !values.oSeo.sSlug) {
          setValue('oSeo.sSlug', data.generateSlugs.oData.sSlug)
          extraSlug && setValue(extraSlug, data.generateSlugs.oData.sSlug)
          clearErrors('oSeo.sSlug')
          setAvailableSlug('')
        }
        if (!data.generateSlugs.oData.sSlug && values.oSeo.sSlug && extraSlug) {
          setValue(extraSlug, values.oSeo.sSlug)
        }
        if (!values.oSeo.sSlug && !data.generateSlugs.oData.sSlug && slug) {
  ...
  function handleChange(e) {
    if (e.target.value && !errors?.oSeo?.sSlug) {
      ...
      if (
        ...
        generateSlugs({ variables: { sSlug: url } })
      }
    }
  }

  useEffect(async () => {
    hidden && !id && slug && generateSlugs({ variables: { sSlug: slug } })
  }, [slug, hidden])
  ...
  function handleInputChange({ target }, name) {
    setValue(`oSeo.oFB.${name}`, target.value)
    setValue(`oSeo.oTwitter.${name}`, target.value)
  }
