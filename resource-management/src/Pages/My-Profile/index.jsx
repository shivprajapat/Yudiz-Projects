...
import { handleAlterImage, projectStatusColor } from 'helpers/helper'
...
export default function MyProfile() {
  ...
  useQuery('myProfile', () => queryClient.getQueryData('myProfile'), {
    select: (data) => {
      return data?.data
    },
    onSuccess: (data) => {
      setData(data?.user)
      setSkills(data?.user?.aSkills)
    },
  })
  const { isLoading, isFetching } = useQuery(['myProfile-projects', requestParams], () => getProfileProjects(requestParams), {
    select: (data) => {
