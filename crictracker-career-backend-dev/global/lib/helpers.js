...
/**
 * Create slug for given model.
 * @param {*} model collection object
 * @param {*} input input value for create slug
 * @param {*} id object id
 * @returns slug value.
 */
_.createSlug = async (model, input, id = '') => {
  input = input.toString().toLowerCase()
    ...
    .replace(/^-+/, '')
    .replace(/-+$/, '')
  const query = { sSlug: input }
  if (id !== '') {
    query._id = { $ne: _.mongify(id) }
  }
  const isSlugExists = await model.findOne(query).lean()
  if (isSlugExists) {
    input = `${input}-${moment(isSlugExists.dCreatedAt).utc().format('DD-MMMM-YYYY')}`
    const isSlugExistsAgain = await model.findOne(query).lean()
 ...
 * @param {*} model job collection
 * @param {*} iJobId job id
 * @param {*} sEmail user email
 * @returns boolean value.
 */

_.CheckJobApply = async (model, iJobId, sEmail) => {
  const query = { iJobId, sEmail }
  const isExists = await model.findOne(query).lean()
