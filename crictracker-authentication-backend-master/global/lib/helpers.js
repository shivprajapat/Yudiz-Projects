...
const parse = require('graphql-parse-fields')
const { invalidDomains } = require('./malaciousdomain')
...
_.asymmetricEncrypt = function (sValue) {
  try {
    return crypt.encrypt(config.PUBLIC_KEY, sValue)
  } catch (error) {
    return error
  }
}
...
_.asymmetricDecrypt = function (sValue) {
  try {
    return crypt.decrypt(config.PRIVATE_KEY, sValue)
  } catch (error) {
    return error
  }
}
...
_.salt = function (length, type) {
  if (config.NODE_ENV !== 'prod') return 1234
  if (type === 'string') {
    return crypto
...
_.throwError = (message, context, prefix) => {
  ...
  if (message === 'authorizationError' || message === 'accountDeactivated' || message === 'accountDeactivated' || (message === 'notFound' && prefix === 'profile')) throw new AuthenticationError(_.stringify({ message: response, status }))
...
/**
 *
 * @param {*} schema Schema of the Api
 * @param {*} populateArray array to be populated
 */

_.projectFields = function (schema, populateArray) {
  const virtualsArray = [
    { original: 'iCategoryId', virtual: 'oCategory' },
    { original: 'iParentId', virtual: 'oParentCategory' },
    { original: 'iParentId', virtual: 'oParentComment' },
    { original: 'iArticleId', virtual: 'oArticle' },
    { original: 'iVideoId', virtual: 'oVideo' },
    { original: 'iCategoryId', virtual: 'oCategory' }
  ]
  const toBePopulatedArray = []
  if (populateArray?.length) {
    Object.keys(schema).forEach((ele) => {
      const index = populateArray.findIndex((elem) => elem === ele)
      if (index > -1) {
        const indexVirtual = virtualsArray.findIndex((element) => element.virtual === ele)
        if (indexVirtual > -1) Object.assign(schema, { [virtualsArray[indexVirtual].original]: true })
        toBePopulatedArray.push({ path: populateArray[index], select: schema[ele] })
        delete schema[ele]
      }
    })
  }

  return { schema, toBePopulatedArray }
}

_.parseFields = (info) => parse(info)

/**
 *
 * @param {*} domain Domain of the email
 * @returns ture or false depending on whether domain is malecious or not
 */

_.validateEmailDomain = (domain) => invalidDomains.includes(domain)
