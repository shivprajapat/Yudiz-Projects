...
import { PUBLIC_KEY, DESIGNATION, ROLES, SEO_REDIRECTS_TYPE_BY_CODE, DESIGNATION_IN_JOB, S3_PREFIX, API_URL } from './constants'
...
export const getAdvanceFeature = (id, isFantasy) => {
  ...
  if (isFantasy) {
    return [...advanceFeatures, { label: <FormattedMessage id='markAsImportant' />, value: 'bImp' }]
  } else {
...
export const handleDragEnd = (result, submenu, menu, setMenu) => {
  ...
  if (type === 'droppable-item') {
    ...
    if (sourceCategoryId === destinationCategoryId) {
      const updatedOrder = reorder(menu.find((category) => category._id === sourceCategoryId)?.[submenu], source.index, destination.index)
      ...
      const sourceOrder = menu.find((category) => category._id === sourceCategoryId)?.[submenu]
      const destinationOrder = menu.find((category) => category._id === destinationCategoryId)?.[submenu]
      ...
      const updatedCategories = menu.map((category) =>
        category._id === sourceCategoryId
          ? { ...category, [submenu]: sourceOrder }
          : category._id === destinationCategoryId
