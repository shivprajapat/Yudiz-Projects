...
import { CUSTOM_URL_WITH_SLASH, S3_PREFIX } from 'shared/constants'
          ...
          <div ref={provided.innerRef} {...provided.draggableProps}>
            <Form.Group className='w-100 form-group d-flex flex-column justify-content-center'>
              <div className='w-100 d-flex align-items-start' style={{ gap: '12px' }}>
                ...
                />
                {
                  image?.url && (imageLoading ? <Button variant="outlined" className='square icon-btn' style={{ width: '37px', height: '37' }}>
                  ...
                  <CommonInput
                    ...
                    required
                >
                  {errors && errors?.category?.[`${categoryIndex}`]?.sName && <Form.Control.Feedback type="invalid">{errors?.category?.[`${categoryIndex}`]?.sName.message}</Form.Control.Feedback>}
                  </CommonInput>
                  {
                    (category?.aSlide === undefined || category?.aSlide?.length < 1) && <CommonInput
                    register={register}
                    errors={errors}
                    ...
                    required
                  >
                    {errors && errors?.category?.[`${categoryIndex}`]?.sSlug && <Form.Control.Feedback type="invalid">{errors?.category?.[`${categoryIndex}`]?.sSlug.message}</Form.Control.Feedback>}
                  </CommonInput>
                  }
                <div className='w-100 d-flex align-items-center' style={{ gap: '12px' }}>
                  <Button variant="light" className='square icon-btn'>
                    {
                      image?.url ? <span className="icon-close" onClick={() => setImage(null)}></span> : <label htmlFor={`category[${categoryIndex}].oImg`} style={{ cursor: 'pointer' }}>
                      <i className="icon-image d-block"></i>
                    </label>
                    }
                  </Button>
                  <Button variant="outline-secondary" size="lg" className='square icon-btn' onClick={(item) => subAppend({ sName: '', sSlug: '' })}>
                    <img src={AddSubmenuIcon} alt="add-submenu-icon" width={20}/>
                  </Button>
                  <Button variant="outline-primary" size="lg" className="square icon-btn" onClick={() => append({ sTitle: '', sUrl: '', aSlide: [] })}>
                    <i className="icon-add d-block" />
                  </Button>
                  {
                    mainMenu?.length > 5 && <Button variant="outline-danger" onClick={() => remove(categoryIndex)} size="lg" className="square icon-btn">
