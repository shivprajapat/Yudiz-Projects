          ...
          <div ref={provided.innerRef} {...provided.draggableProps}>
            <div className='d-flex align-items-start justify-content-center' style={{ marginLeft: '24px', gap: '12px' }}>
              <CommonInput
                  ...
                  required
              >
                {errors && errors?.category?.[`${categoryIndex}`]?.aSlide?.[`${index}`]?.sName && <Form.Control.Feedback type="invalid">{errors?.category?.[`${categoryIndex}`]?.aSlide?.[`${index}`]?.sName.message}</Form.Control.Feedback>}
              </CommonInput>
              <CommonInput
                  register={register}
                  errors={errors}
                ...
                validation={{
                  ...
                  required
              >
                {errors && errors?.category?.[`${categoryIndex}`]?.aSlide?.[`${index}`]?.sSlug && <Form.Control.Feedback type="invalid">{errors?.category?.[`${categoryIndex}`]?.aSlide?.[`${index}`]?.sSlug.message}</Form.Control.Feedback>}
                </CommonInput>
