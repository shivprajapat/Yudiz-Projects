...
const HeaderDraggableMenu = ({ category, categoryIndex, append, remove, control, register, errors, setValue, mainMenu, watch }) => {
  ...
  return (
    <Draggable draggableId={category?._id} index={categoryIndex}>
      {(provided, snapshot) => {
        return (
            ...
            <Form.Group className='d-flex flex-column justify-content-center'>
              <div className='d-flex align-items-start' style={{ gap: '12px' }}>
                <CommonInput
                  ...
                  required
                >
                {errors && errors?.category?.[`${categoryIndex}`]?.sTitle && <Form.Control.Feedback type="invalid">{errors?.category?.[`${categoryIndex}`]?.sTitle.message}</Form.Control.Feedback>}
                </CommonInput>
                {
                  (category?.oChildren === undefined || category?.oChildren?.length < 1) && <CommonInput
                  register={register}
                  errors={errors}
                ...
                <div className='w-100 d-flex align-items-center' style={{ gap: '12px' }}>
                  <Button variant="outline-secondary" className='square icon-btn' size="lg" onClick={(item) => subAppend({ sTitle: '', sSlug: '' })}>
                    <img src={AddSubmenuIcon} alt="add-submenu-icon" width={20}/>
                  </Button>
                  <Button variant="outline-primary" size="lg" className="square icon-btn" onClick={() => append({ sTitle: '', sSlug: '', oChildren: [] })}>
