...
function DisplayAuthor({ control, disabled, articleData, type }) {
  const { id } = useParams()
  ...
  function handleChange(e) {
    if (id) {
      if (type === 'a') articleAuthorChange({ variables: { input: { iArticleId: articleData._id, iAuthorDId: e._id } } })
      else fantasyArticleAuthorChange({ variables: { input: { iArticleId: articleData._id, iAuthorDId: e._id } } })
    }
  }
    ...
    <Controller
      ...
      render={({ field: { onChange, value = [] } }) => (
        <Select
          ...
          className="react-select display-author on-hover arrow"
