...
import { GET_FEEDBACK_QUERY_TYPE } from '@graphql/feedback/feedback.query'
...
const FeedBack = ({ seoData }) => {
  ...
  useQuery(GET_FEEDBACK_QUERY_TYPE, {
    onCompleted: (data) => {
      if (data && data.getFeedbackQueryType) {
        setTypeOfQueryOptions(data.getFeedbackQueryType)
      }
    }
  })
              ...
              <div className="common-box">
                <Form autoComplete="off" className="m-2 m-md-3" onSubmit={handleSubmit(onSubmit)}>
                  <Row>
                    ...
                    <Col md={12}>
                      <Form.Group className={`${formStyles.formGroup}`} controlId="sPageLink">
                        <Form.Label className={`${formStyles.label}`}>
