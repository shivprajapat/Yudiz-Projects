...
function CurrentMatches({ data, status, filterData }) {
  ...
  const payloads = useRef({ eCategory: 'i', eStatus: status, sTimezone: getTimeZone(), iTeamId: null, iSeriesId: null, iVenueId: null })
  ...
  const handleCategoryFilter = (value) => {
    ...
    payloads.current = { ...payloads.current, eCategory: value, eStatus: status }
    ...
    <div className="position-relative">
      ...
      <div className={`${mobilefilter.mobFixtureFilter} ${toggleFilter && mobilefilter.active}`}>
        ...
        <div className={`${mobilefilter.innerBlock} d-block d-md-none`}>
          <NameFilter formStyles={formStyles} mobilefilter={mobilefilter} handleClear={handleClear} filterData={newFilterData} handleFilterChange={handleFilterChange}/>
        </div>
      </div>
      <Button
        variant="link"
      ...
      <div className={`${mobilefilter.fixturesTab} ${mobilefilter.lightTab}`}>
        <Nav className={`${navStyles.commonNav} text-uppercase equal-width-nav`} variant="pills">
          ...
          <Nav.Item className={`${navStyles.item}`}>
            <Link href={allRoutes.fixturesUpcoming} prefetch={false}>
          ...
          <Nav.Item className={`${navStyles.item}`}>
            <Link href={allRoutes.fixturesRecent} prefetch={false}>
