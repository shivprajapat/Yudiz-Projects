...
const KeyStats = ({ tossData, liveScoreData }) => {
  ...
  return (
    <section className={`${styles.keyStats} common-box p-0 mb-3`}>
      <div className={`${styles.head}`}>
        <p className="text-uppercase text-primary font-bold mb-0">{t('common:KeyStats')}</p>
      </div>
      <div className={`${styles.content} d-flex flex-wrap justify-content-between`}>
        <p
          className={`${styles.item} ${styles.halfItem} text-secondary col-6 col-xl-12 d-flex flex-column flex-sm-row justify-content-between`}
        >
        ...
        <p className={`${styles.item} text-secondary col-lg-12 col-12 d-flex flex-column flex-sm-row justify-content-between`}>
