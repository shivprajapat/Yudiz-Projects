...
const EndOver = ({ data, id, inning }) => {
  ...
  return (
    <>
      <div className={`${styles.over} bg-info common-box d-flex flex-wrap flex-xl-nowrap align-items-xl-center`} id={id}>
        <div className={`${styles.info} d-none d-md-flex flex-xl-column align-items-center text-xl-center`}>
          ...
          <p className="mb-0 ms-1 big-text font-bold">{data?.sOver}</p>
        </div>
        <div className={`${styles.info} text-xl-center flex-grow-1`}>
          <p className="mb-1">{t('common:RunScored')}: {data?.nRuns}</p>
          <p className="mb-0 big-text font-bold">{data?.aOverScores?.map((ball, index) => <span key={index}>{index ? ' ' : ''}{ball}</span>)}</p>
        </div>
        <div className={`${styles.info} text-end text-xl-center flex-grow-1`}>
          ...
          <p className="mb-0 big-text font-bold">{inning.map((i) => i?.nInningNumber === data?.nInningNumber && i?.sShortName)} - {data?.sScore}</p>
        </div>
          ...
          {data?.aBatters?.map((bat, index) => {
            return (
              <p className="mb-1 d-flex justify-content-xl-between" key={index}>
                {bat?.oBatter?.sFullName || bat?.oBatter?.sShortName} <b>{bat?.nRuns}({bat?.nBallFaced})</b>
              </p>
            )
          })}
        </div>}
