...
const MatchDetailOvers = ({ data, matchDetail }) => {
  ...
  const [getMoreOvers, { data: updatedOverData, loading: overLoading }] = useLazyQuery(RECENT_OVER)
  ...
  const handleLoadMore = () => {
    ...
    getMoreOvers({ variables: { input: { ...payloads.current } } })
    setEnableBtn(false)
  ...
  useEffect(() => {
    if (tabChange.current) {
      updatedOverData && setOverData(updatedOverData?.listMatchOvers?.aResults)
      tabChange.current = false
    } else {
      updatedOverData && setOverData([...overData, ...updatedOverData?.listMatchOvers?.aResults])
    }
  }, [updatedOverData])
  ...
  return (
    <>
        ...
        {matchDetail?.aInning?.map((inning, index) => {
          return (
            <Nav.Item className={navStyles.item} key={index} onClick={() => handleInning(inning)}>
              <a className={`${selectedTab === inning?.nInningNumber && 'active'} nav-link`}>
                {inning?.sShortName}
              </a>
            </Nav.Item>
          )
      ...
      {
        enablebtn && (total.current > 3) && <div className="text-center my-2">
