            ...
            {savedArticles?.map((article) => {
              return (
                <div key={article?._id} className={styles.item}>
                  <ArticleSmall data={getArticleData(article)} isLarge={true} isVideo={article?.oVideo?._id && true} />
                  <Button variant="link" onClick={() => handleConfirm(article?._id)} className={`${styles.closeMenu} btn-close`}></Button>
                </div>
              )
            })}
