        ...
        video?.map((vid, ind) => {
          return (
            <React.Fragment key={vid._id}>
              ...
              <section id={vid._id} className={`${styles.homeArticles}`}>
                <h4 className="line-title">
                  <Link href={vid?.oCategory?.eType === 's' ? `/${vid?.oSeo?.sSlug}/?tab=videos` : allRoutes.seriesVideos(`/${vid?.oSeo?.sSlug}/`)} prefetch={false}>
                    <a><span className="d-block text-nowrap overflow-hidden">{vid?.sName}</span></a>
            ...
            <NotificationIcon />
                ...
                {vid.aVideos.map((ar) => {
                  if (ar.sType === 'nBig') {
                    ...
                    return (
                      <Row className={`row-8 common-box mb-2 ${styles.articleGridList} ${styles.articleGroup}`} key={ar.sType}>
                        {ar.mappedArticle.map((g) => {
                          return (
                            <Col key={g._id} sm={6}>
                              <ArticleGrid isVideo data={g} />
                ...
                <Link href={vid?.oCategory?.eType === 's' ? `/${vid?.oSeo?.sSlug}/?tab=videos` : allRoutes.seriesVideos(`/${vid?.oSeo?.sSlug}/`)} className="theme-btn full-btn btn btn-primary" prefetch={false}>
