      ...
      <div hidden={!isVisible} className={`${style?.popupAd} ${isVisible ? 'd-flex ' : ''}align-items-center justify-content-center`}>
        <div className={style.popupAdInner}>
          <div className={`${style.adHeader} w-100 text-end`}>
            <span className={`text-white ${style.count}`}> <Trans i18nKey="common:AdCloseIn" /> - 00:00:{timer > 9 ? timer : `0${timer}`}</span>
            <Button onClick={() => handleClose()} variant="link" className="btn-close bg-white rounded" />
          </div>
          <div id={id} />
        </div>
      </div>
