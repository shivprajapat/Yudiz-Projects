...
const SearchCard = ({ data }) => {
  ...
  return (
    <>
      ...
      a{color:inherit;text-decoration:none}.d-flex{display:flex;-webkit-display:flex}.align-items-center{align-items:center;-webkit-align-items:center}.common-box{margin-bottom:24px;padding:16px;background:#fff;border-radius:16px}.common-box>:last-child{margin-bottom:0}.searchCard .icon{margin-right:8px;width:60px;border-radius:50%;overflow:hidden}h3{margin:0px 0px 2px;font-size:18px;line-height:24px}p{margin:0;font-size:14px;line-height:20px}@media(max-width: 1199px){h3{font-size:17px;line-height:23px}}@media(max-width: 767px){.common-box{margin-bottom:20px;padding:12px;border-radius:12px}h3{font-size:16px;line-height:22px}p{margin:0;font-size:13px;line-height:18px}}/*# sourceMappingURL=style.css.map */

      `}
      </style>
      <div className="searchCard common-box d-flex align-items-center mb-2 mb-md-3 mb-xl-4">
        <div className="icon rounded-circle overflow-hidden me-2">
          {data?.oSeo?.eType === 'p' && (
            <amp-img
              src={getImgURL(data?.oImg?.sUrl) || '/static/player-placeholder.jpg'}
              alt={data?.sFullName}
              layout="responsive"
              width="40"
              height="40"
