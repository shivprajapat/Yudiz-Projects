...
const CategoryVideos = ({ data, nTotal }) => {
          ...
          return (
            <section key={category?._id} className="common-section pt-3" id={category?._id}>
              <div className={'mb-3 d-flex justify-content-between align-items-center'}>
                ...
                <Link href={`${category?.oSeo?.sSlug}/${category?.oCategory?.eType === 's' ? '?tab=videos' : category?.oSeo?.eType !== 'pl' ? `${allRoutes.cricketVideo}` : ''}`} prefetch={false}>
                  <a className={`${styles.link} text-primary font-bold text-uppercase`}>{t('common:ViewAll')}</a>
                </Link>
              </div>
