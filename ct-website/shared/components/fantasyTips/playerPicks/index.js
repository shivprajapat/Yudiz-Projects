...
const PlayerPicks = ({ fantasystyles, playerPicksData }) => {
      ...
      <Row>
        {selectedTab === 'captains' &&
          playerPicksData?.aCVCFan?.map((captainData, index) => (
            <SinglePlayerPick styles={styles} key={index} type={selectedTab} data={captainData} />
          ))}
        {selectedTab === 'top' &&
          playerPicksData?.aTopicPicksFan?.map((topPickData, index) => (
            <SinglePlayerPick styles={styles} key={index} type={selectedTab} data={topPickData} />
          ))}
        {selectedTab === 'budget' &&
          playerPicksData?.aBudgetPicksFan?.map((budgetData, index) => (
            <SinglePlayerPick styles={styles} key={index} type={selectedTab} data={budgetData} />
          ))}
      </Row>
    </section>
  )
