...
const QualificationsModel = require('../qualification/qualification.model')
const UniversitiesModel = require('../university/university.model')
...
class CounsellorService {
  ...
  async createCounsellor(req, res) {
    try {
      ...
      const userExist = await CounsellorModel.findOne({ where: { [Op.or]: [{ email: email }, { mobile: mobile }, { user_name: sUsername }], deleted_at: null } })
      ...
      try {
        ...
        const counsellor_status_val = counsellor_status || 'approved'
        const counsellor_commission = commission || 0
        const counsellor = await CounsellorModel.create({ custom_id: sCustomId, ollato_code: getOllatoNumber, center_id, counselor_id: counsellor_id, slug: sUsername, user_name: sUsername, password: sPassword, first_name, middle_name, last_name, email, mobile, career_counsellor, psychologist, overseas_counsellor, subject_expert, OTP, dob, gender, mother_name, father_name, verified_at: dVerifiedAt, commission: counsellor_commission, status: counsellor_status_val }, { transaction })
        counsellor.setDataValue('password', null)

        const sCustomIdCounsellorDetail = randomStr(8, 'string')
        await CounsellorDetailModel.create({ custom_id: sCustomIdCounsellorDetail, counsellor_id: counsellor.getDataValue('id'), resume, professional_expertness, country_id, state_id, city_id, pin_code, high_qualification_id, high_university_id, last_qualification_id, last_university_id, certificate_qualification_id, certificate_university_id, experience_year, experience_month, pan_number, gst_no }, { transaction })
  ...
  async updateCounsellor(req, res) {
    try {
      ...
      if (exist.center_id !== null) {
        commission = 0
      }
      try {
        if (updateType && updateType === 'status') {
          await CounsellorModel.update({ is_active: isActive }, { where: { id: counsellor_id } })
      ...
      try {
        ...
        let centerId = null
        if (center_id !== undefined && center_id !== null && center_id !== 'null') centerId = parseInt(center_id)
        console.log('centerId', centerId)

        const counsellor_status_val = counsellor_status || 'approved'
        const counsellor_commission = commission || 0

        await CounsellorModel.update({ center_id: centerId, counselor_id: counsellor_id, slug: sUsername, user_name: sUsername, first_name, middle_name, last_name, email, mobile, career_counsellor, psychologist, overseas_counsellor, subject_expert, dob, mother_name, father_name, status: counsellor_status_val, commission: counsellor_commission }, { where: { id: counsellor_id } }, { transaction })

        await CounsellorDetailModel.update({ resume, professional_expertness, country_id, state_id, city_id, pin_code, high_qualification_id, high_university_id, last_qualification_id, last_university_id, certificate_qualification_id, certificate_university_id, experience_year, experience_month, pan_number, gst_no }, { where: { counsellor_id: counsellor_id } }, { transaction })
