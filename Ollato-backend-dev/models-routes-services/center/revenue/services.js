...
const CenterRevenueModel = require('./center_revenue.model')
const CenterModel = require('../Auth/center.model')
const { Op, Sequelize } = require('sequelize')

class CenterRevenueServices {
  async getCenterRevenue(req, res) {
    try {
      removenull(req.body)
      ...
      const revenue = await CenterRevenueModel.findAll(query)
      const remaining_amount = await CenterModel.findOne({ where: { id: req.user.id }, attributes: ['remaining_amount'] })
...
module.exports = new CenterRevenueServices()
