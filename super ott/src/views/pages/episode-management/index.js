...
import { getEpisodeList, episodeUpdate } from '../../../redux/actions/episode'

const EpisodeManagement = () => {
  const dispatch = useDispatch()
  const history = useHistory()
  const { allEpisodes, totalEpisodes } = useSelector((state) => state.episode)
  ...
  const handleUpdateEpisode = (id, status) => {
    dispatch(episodeUpdate(id, { eStatus: status ? 'y' : 'n' }))
  }
      ...
      cell: (row) => {
        return (
          <CustomInput
            ...
            onChange={(e) => {
              handleUpdateEpisode(row?._id, e.target.checked)
            }}
            inline
          />
        )
        ...
        return (
          <div className="d-flex ">
            <div
              ...
              onClick={() => {
                history.push(`/episode-management/view/${row?._id}`)
              }}
            >
              <Eye size={20} />
            </div>
  ...
  useEffect(() => {
    dispatch(getEpisodeList(requestParams))
  }, [requestParams])
  ...
  const CustomPagination = () => {
    const count = Math.ceil(totalEpisodes / requestParams.size)
    return (
      <ReactPaginate
        previousLabel={''}
        nextLabel={''}
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4">Episode Management</CardTitle>
        </CardHeader>
