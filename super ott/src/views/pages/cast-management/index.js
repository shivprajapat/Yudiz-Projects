...
import { deleteCastAction, getCastList, castUpdate } from '../../../redux/actions/cast'
import { convertDateInDMY, debounce, limitOptions, src, statusOptions } from '../../../utility/Utils'

const CastManagement = () => {
  const dispatch = useDispatch()
  const { allCast, totalCast } = useSelector((state) => state.cast)
  ...
  const handleUpdateCast = (id, status) => {
    dispatch(castUpdate(id, { eStatus: status ? 'y' : 'n' }))
  }
        ...
        return (
          <div>
            <CustomInput
              ...
              onChange={(e) => {
                handleUpdateCast(row?._id, e.target.checked)
              }}
            />
          </div>
        )
        ...
        return (
          <div className="d-flex">
            <div
              ...
              onClick={() => {
                history.push(`/castmanagement/update/${row?._id}`)
              }}
            >
              <Edit size={20} />
            </div>
  ...
  useEffect(() => {
    dispatch(getCastList(requestParams))
  }, [requestParams])
  ...
  const handleDeleteAction = () => {
    dispatch(
      deleteCastAction(selectedCast, (data) => {
  ...
  const CustomPagination = () => {
    const count = Math.ceil(totalCast / requestParams.size)
    ...
    <Fragment>
      <Card>
        <CardHeader className="flex-md-row flex-column align-md-items-center align-items-start border-bottom">
          <CardTitle tag="h4">Cast Management</CardTitle>
          <div className="d-flex mt-md-0 mt-1">
            <Button className="ml-2" color="primary" onClick={() => history.push('/castmanagement/add')}>
              <Plus size={15} />
              <span className="align-middle ml-50">{'Add Cast'}</span>
            </Button>
          </div>
        </CardHeader>
