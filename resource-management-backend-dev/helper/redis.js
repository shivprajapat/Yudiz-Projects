...
async function checkLoginLimitByIp(req, res, next) {
  try {
    ...
    if (ipLimit > config.threshold) {
      ...
      if (ttl === -1) {
        await redisClient.expire(`${path}:${ip}`, 3600)
      }
      return next()
    }
  } catch (error) {
...
module.exports = { checkLoginLimitByIp, queuePush, queuePop, getDataUsingScore, deleteByScore, checkDownLoadExcelLimitByIp, redisClient, redisClient2 }
