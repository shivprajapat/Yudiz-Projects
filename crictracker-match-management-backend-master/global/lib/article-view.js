...
/**
 *Same user browsing same fantasy article expiration time
*/
const ipExpireSec = IP_EXPIRE_SEC

const fantasyArticleViews = async (article, ip) => {
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        const key = 'fantasyarticle:' + article._id + ':views'
        let redisViewCount = 0
        if (ipLimit) {
          // When an IP accesses the post for the first time in 300 seconds, refresh the page views of the fantasy article
          const articleViews = await redisclient.hincrby('fantasyarticles', key, 1, function (error, reply) {
            if (error) reject(error)
            return reply
          })
          // For most view fantasy article set
          if (articleViews) {
          ...
          const oldViews = await redisclient.hget('fantasyarticles', key, function (error, reply) {
            if (error) reject(error)
            return reply
          })
          if (oldViews) {
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        const ipArticleViewKey = 'fantasyarticle:ip:limit:' + id
...
module.exports = { fantasyArticleViews }
