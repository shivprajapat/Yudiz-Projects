...
_.encodeString = function (data, len) {
  return crypto.createHmac('sha256', data).update(data).digest('hex').slice(0, len)
}
...
_.stripSpaceAndLowercase = (string) => {
...
_.sortArrayOfObj = (data, key, sortType = 'asc') => {
  return data.sort((a, b) => sortType === 'asc' ? a[key] - b[key] : b[key] - a[key])
}

_.createPollTitle = (match) => {
  const array = [
