...
services.removeImagePath = async (fileKey) => {
  return new Promise((resolve, reject) => {
    const params = {
      Bucket: config.S3_BUCKET_NAME,
      Key: fileKey
    ...
    return resolve(s3.send(command))
  })
}

services.getS3Object = async (s3Path) => {
  try {
    const s3Params = {
      Bucket: config.S3_BUCKET_NAME,
      Key: s3Path // path to the object you're looking for
    }
...
services.getSize = async (s3Path, image) => {
  try {
    const params = { Key: s3Path, Bucket: config.S3_BUCKET_NAME }
    const command = new HeadObjectCommand(params)
    return await s3.send(command)
  } catch (error) {
